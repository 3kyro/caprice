...
// caprice.run() will run the caprice console in a separate thread.
...
// For this example we will simply print back the tokens send by caprice
loop {
    ...
    if let Ok(token) = rx.try_recv() {
        match token.as_str() {
            ...
            // send back the token to be printed
